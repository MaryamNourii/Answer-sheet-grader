...
class AnswerSheetGrader():
  def __init__(self, img_path, height, width, max_qstn):
  ...
  def rotate_img(self):
    ...
    elif (abs(self.width-self.big_circle[0])>(0.9 * self.width) and abs(self.height-self.big_circle[1])>(0.9 * height)):
      self.mnplt_img = cv2.rotate(self.mnplt_img, cv2.ROTATE_180)

    elif (abs(self.width-self.big_circle[0])>(0.9 * self.width) and abs(self.height-self.big_circle[1])<(0.1 * height)):
  ...
  def split_main_img(self):
    ...
    self.answers_roi = self.mnplt_img[min_XY[1]:,:]
    self.rects_roi = self.mnplt_img[min_XY[1]-10:,0:min_XY[0]-10]
  ...
  def find_left_rects(self):
    ...
    rect_roi = self.rects_roi.copy()
    gray_img = cv2.cvtColor(rect_roi, cv2.COLOR_BGR2GRAY)
    ...
    #         cv2.rectangle(self.rects_roi,(x,y),(x+w,y+h),(0, 0, 255),2)
  ...
  def find_main_bubbles_pos(self):
    ...
    i=40
    j=8
    sec1 = answers_roi_img[self.rects_cnt[i]+25:self.rects_cnt[i]+38,self.vertical_cnt[j]+14:self.vertical_cnt[j+1]]

    sec_w = int(sec1.shape[0]*1.3)
    sec_h = int(sec1.shape[1]*1.3)
    sec1 = cv2.resize(sec1, (sec_h, sec_w))
  ...
  def get_answrs(self):
    ...
    for i in range(1,len(self.vertical_cnt)-2):
      if qstn_ind > self.max_qstn: 
        break
      sec1 = answers_roi_img[self.rects_cnt[0]:,self.vertical_cnt[i]+13:self.vertical_cnt[i+1]]
      sec_w = int(sec1.shape[0]*1.3)
      sec_h = int(sec1.shape[1]*1.3)
      ...
      for c in contours_O:
          ...
          if len(box) == 4:
              ...
              for rct_snwr in rects_answer:
                if (y > rct_snwr[1]-5 and y < rct_snwr[1]+5):
                    y = rct_snwr[1]


              if [x,y,w,h] not in rects_answer and answr != -1 and qstn_ind < self.max_qstn: 
                rects_answer.append([x,y,w,h])
                self.answer[qstn_ind]=[answr,x,y]
                cv2.rectangle(so,(x,y),(x+w,y+h),(0, 0, 255),1)
                qstn_ind += 1
      # cv2_imshow(so)    
      return self.answer 
...
def main():
  asg = AnswerSheetGrader('Images\1.jpg', 800, 700, 170)
  asg.pre_processing()
